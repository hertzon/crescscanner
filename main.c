        ...
        case BLE_GAP_EVT_DISCONNECTED:
            ...
            break;
				case BLE_GAP_EVT_ADV_REPORT:
						...
						printf("\r\nReport data: ");
						uint8_t dataLenght=p_ble_evt->evt.gap_evt.params.adv_report.dlen;
						printf("\r\nLenght: %u\r\n",dataLenght);
						char datos[50]={0};
						bool is=false;
						for (int i=0;i<sizeof(p_ble_evt->evt.gap_evt.params.adv_report.data);i++){
							datos[i]=p_ble_evt->evt.gap_evt.params.adv_report.data[i];
							//printf("%02X",datos[i]);	
		
						}
						if (dataLenght>17 && datos[18]=='B' && datos[19]=='C' && datos[20]=='N'){
								//printf("%c",datos[18]);printf("%c",datos[19]);printf("%c",datos[20]);
						...
						peer_addr=p_ble_evt->evt.gap_evt.params.adv_report.peer_addr;
						printf("MAC: ");
						for (int j=0;j<sizeof(peer_addr.addr);j++){
								printf("%02X",peer_addr.addr[j]);
						}
...
//						}
						...
						//}
//						printf("\r\n");
//						if (peer_addr.addr[5]==0xD4 && peer_addr.addr[4]==0x0d && peer_addr.addr[3]==0x02 && peer_addr.addr[2]==0x1a && peer_addr.addr[1]==0xe5 && peer_addr.addr[0]==0x62){
//								printf("BLE_GAP_EVT_ADV_REPORT\n");
//								printf("RSSI(2): %d\n",p_ble_evt->evt.gap_evt.params.adv_report.rssi);
//						}
						//data_t  data_manuf;
						
						//adv_report_parse(BLE_GAP_AD_TYPE_16BIT_SERVICE_UUID_MORE_AVAILABLE, &adv_data, &data_manuf);
						//for (int i=0;i<data_manuf.data_len;i++){
							//printf("%02X",data_manuf.p_data[i]);
